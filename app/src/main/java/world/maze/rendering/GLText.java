...
public class GLText {
	...
	private int mProgramHandle; 					   // OpenGL Program handle
	...
	public GLText(int programHandle, AssetManager assets) {
		mProgramHandle = programHandle;
		this.assets = assets;                           // Save the Asset Manager Instance
		
		batch = new SpriteBatch(CHAR_BATCH_SIZE, programHandle );  // Create Sprite Batch (with Defined Size)
	...
	public boolean load(String file, int size, int padX, int padY) {
		...
		paint.setColor(0xff000000);                   // Set ARGB (Black, Opaque)
